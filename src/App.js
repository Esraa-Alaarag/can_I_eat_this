...
class App extends Component {
  ...
  addAllergies(){
     console.log(this.state.issues);
    axios.post('https://can-i-eat-this-api.herokuapp.com/api/allergies/', {
  ...
  axios.get(`https://can-i-eat-this-api.herokuapp.com/api/allergies/${this.state.userid}`)
    .then((res) => {
    var issues = [res.data.data.eggsallergy, res.data.data.fishallergy,
                  res.data.data.milkallergy, res.data.data.peanutsallergy, 
                  res.data.data.sesameallergy, res.data.data.shellfishallergy,
                  res.data.data.soyallergy, res.data.data.treenutsallergy,
                  res.data.data.wheatallergy];
                  console.log(issues);
    this.setState ({
      readOnly: true,
      issues: issues
  ...
  updateAllergies(){
    // console.log(this.state.issues);
    axios.put(`https://can-i-eat-this-api.herokuapp.com/api/allergies/${this.state.userid}`, {
      userid: this.state.userid,
      eggsallergy: this.state.issues[0] === true ? true : false,
      fishallergy: this.state.issues[1] === true ? true : false,
      milkallergy: this.state.issues[2] === true ? true : false,
