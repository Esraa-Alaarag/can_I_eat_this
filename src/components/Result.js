...
class Result extends Component {
  ...
  constructor(props) {
    ...
    this.getIngred = this.getIngred.bind(this);
      ...
      getIngred() {
        var upc=this.props.match.params.upc;
        ...
        const appKey = 'e2eff610c0e3255bbe2f14dbfe91f48c'; //app one
  ...
  grabProductImage() {
    var image;
    $.ajax({url: "https://api.walmartlabs.com/v1/search?apiKey=nreuk5qdfbhbmwheh4emmhf7&query="+this.props.match.params.upc,
     jsonp: "callback",
      dataType: "jsonp",
...
//   }
  ...
  addProduct() {
    console.log(this.state.userid + this.state.productName + this.state.upc + this.state.eggs + this.state.fish + this.state.milk + this.state.peanuts + this.state.sesame + this.state.shellfish + this.state.soy + this.state.treenuts + this.state.wheat + this.state.image + this.state.result )
...
console.log("result is alive");
    ...
    if (this.state.productName === undefined) {
      return (
        <center>
        <img src="/images/loader-2_food.gif" alt="loading" />
        </center>
        )
     } 
     else if (this.state.ingredientList === undefined) {
      return (
        <FourOFour/>
        )
     }
